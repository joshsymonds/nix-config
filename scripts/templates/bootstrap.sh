#!/usr/bin/env bash
#
# bootstrap.sh — install-time script that runs on the target NixOS minimal ISO.
#
# Lives in the kit USB alongside the flake tarball and identity files. The
# user boots the target on a stock NixOS minimal ISO, plugs the USB, and runs:
#
#   sudo ./bootstrap.sh
#
# Reads manifest.env for HOSTNAME and FLAKE_REF. Asks the user to choose the
# target disk from a candidate list (DISK can't be baked at prep time — we
# don't have the hardware until install day). Confirms a destructive WIPE,
# substitutes the disko sentinel, runs disko, takes the impermanence
# @root-blank snapshot, copies identity files into /mnt, runs nixos-install.

set -euo pipefail

# Test-overridable paths so the bats harness can redirect into fixtures.
KIT_DIR="${KIT_DIR:-$(dirname "$(readlink -f "$0")")}"
MNT="${MNT:-/mnt}"
...
choose_disk() {
  ...
  while IFS= read -r line; do
    ...
    # Skip disks that have any mounted partition.
...
Next:
  1. Reboot. Remove the install ISO USB. Keep the kit USB until first
     boot succeeds (it's your recovery copy of the host keys).
  2. First boot prompts for LUKS passphrase (you set it during disko).
  3. Lanzaboote signs the kernel; SecureBoot keys auto-enroll on first
     boot via sbctl. If SecureBoot is in setup mode, it switches to
...
main() {
  ...
  # Sourced inside main() so the bats harness can `source bootstrap.sh`
